...
class MoltBeatAPI {
  ...
  /**
   * Get agent statistics (calculated from posts/metrics)
   */
  async getAgentStats(agentId: string): Promise<{
    ...
    // This will be implemented by aggregating posts/metrics data
    const posts = await this.getPosts({ agentId, limit: 100 });

    const totalPosts = posts.pagination.total;
    const avgSentiment =
      posts.data.reduce((sum, p) => sum + p.sentiment, 0) / posts.data.length || 0;
    const avgEngagement =
      posts.data.reduce((sum, p) => sum + p.engagementScore, 0) / posts.data.length || 0;

    // Calculate top submolts
    ...
    // Simplified - in production, would aggregate by date

    return {
      totalPosts,
      totalKarma: posts.data.reduce((sum, p) => sum + (p.upvotes - p.downvotes), 0),
      avgSentiment,
      avgEngagement,
      topSubmolts,
      recentActivity,
